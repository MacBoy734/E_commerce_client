...
function SearchPageContent() {
    ...
    const fetchResults = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/products/search?q=${query}`);
            if (!res.ok) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                {products.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center min-h-[60vh]">
                        <p className="text-lg text-white">No Products were Found! please try searching something else.</p>
                    </div>
                )}
            </div>
        </div>
